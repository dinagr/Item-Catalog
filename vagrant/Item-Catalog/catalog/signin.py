...
def showLogin():
    ...
    itemsByCategory = app.db.query(Item.name, Category.name).filter(Item.category_id==Category.id).order_by(Item.timestmp.desc()).limit(9)
    # return "The current session state is %s" % login_session['state']
    return render_template('login.html', categories = categories, items = itemsByCategory, STATE=state, login_session = login_session)
